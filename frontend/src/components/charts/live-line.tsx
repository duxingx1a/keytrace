...
export interface LiveLineProps {
  ...
  fill?: boolean;
  /** Fade edges of the line/fill. Default: true */
  fadeEdges?: boolean;
  ...
  return (
    <>
      ...
      {fill && data.length > 1 && (
        fadeEdges ? (
          <g mask={`url(#${fadeMaskId})`}>
            <AreaClosed curve={curve} data={data} fill={`url(#${areaGradientId})`}
              strokeWidth={0} x={getX} y={getY} yScale={yScale} />
          </g>
        ) : (
          <AreaClosed curve={curve} data={data} fill={`url(#${areaGradientId})`}
            strokeWidth={0} x={getX} y={getY} yScale={yScale} />
      ...
      {data.length > 1 && (
        fadeEdges ? (
          <g mask={`url(#${fadeMaskId})`}>
            <LinePath curve={curve} data={data} stroke={`url(#${gradientId})`}
      ...
      >
        ...
        {/* Badge — placed above the dot to avoid right-side overflow */}
        {badge && (
          <g transform={`translate(${liveDotX},${liveDotY - 20})`}>
            <rect
              ...
              x={-((formatValue(liveValue).length * 7.5 + 16) / 2)}
              y={-12}
            />
            <text
              fill={chartCssVars.badgeForeground}
              fontFamily="var(--chart-font-mono)"
              fontSize={11}
              fontWeight={500}
              textAnchor="middle"
              x={0}
