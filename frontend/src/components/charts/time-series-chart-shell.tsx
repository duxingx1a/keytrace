...
export interface TimeSeriesChartInnerProps {
  ...
  /** Custom label formatter for x-axis ticks. Falls back to shortDateFmt. */
...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const dateLabels = useMemo(
    () => visiblePlotData.map((d) => (labelFormatter ?? shortDateFmt.format)(xAccessor(d))),
    [visiblePlotData, xAccessor, labelFormatter]
  );
